...
public class Amazon_Testcase12 extends Launch_Quit_Automation_Amazon {
	...
	public void verify_shopping_cart(String username, String password) throws EncryptedDocumentException, IOException, InterruptedException {
		...
		p1.creditcard();
		p1.credit_card_selection_check();	//assert
		
		Thread.sleep(2000);
		p1.debit();
		p1.debit_card_selection_check();	//assert
		Thread.sleep(2000);
		p1.net_bank();
		p1.net_banking_selection_check();	//assert
		Thread.sleep(2000);
		p1.upi();
		p1.upi_selection_check();	//assert
		Thread.sleep(2000);
		p1.emi();
		p1.emi_selection_check();	//assert
		Thread.sleep(2000);
		p1.cod();
		p1.cod_selection_check();	//assert
		Thread.sleep(2000);
		n1.checkout_page();
	//	Amz_logout01 log2=new Amz_logout01(driver);
		log2.mouse_hover_account(driver);
